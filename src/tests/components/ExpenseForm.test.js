...
test(
    'should render expense form with data correctly',
    () => {
        const wrapper = new shallow(<ExpenseForm expense={expenses[0]} />);
        expect(wrapper).toMatchSnapshot();
    }
);
...
test(
    'should set description on the input change',
    () => {
        const wrapper = shallow(<ExpenseForm />);
...
test(
    'should set amount on input change',
    () => {
        const wrapper = shallow(<ExpenseForm />);
        const value = '23.50';
...
test(
    'should not set amount on input change with invalid input',
    () => {
        const wrapper = shallow(<ExpenseForm />);
        const value = '12.122';
        const prevousValue = wrapper.state('amount');
        wrapper.find('input').at(1).simulate('change', {
            target: { value }
        });
        expect(wrapper.state('amount')).toBe(prevousValue);
